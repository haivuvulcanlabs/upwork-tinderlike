...
//  upwork-match
//
//  Created by vulcanlabs-hai on 13/12/2022.
//
...
class AWSS3Manager {
    ...
    private init () { }
    let bucketName = AWSS3Manager.AWS_BucketName //5
...
//        let fileName: String = ProcessInfo.processInfo.globallyUniqueString + (".jpeg")
    ...
    private func uploadfile(fileUrl: URL, fileName: String, contenType: String, progress: progressBlock?, completion: completionBlock?) {
        ...
        completionHandler = { (task, error) -> Void in
            DispatchQueue.main.async(execute: {
                if error == nil {
                    ...
                    let publicURL = url?.appendingPathComponent("uploads/" + fileName)
        ...
        awsTransferUtility.uploadFile(fileUrl, bucket: bucketName, key: "uploads/" + fileName, contentType: contenType, expression: expression, completionHandler: completionHandler).continueWith { (task) -> Any? in
